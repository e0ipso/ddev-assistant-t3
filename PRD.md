...
The existing T3 setup works as a devcontainer feature, but DDEV projects need an equivalent integration that follows DDEV conventions. T3 must run where assistant CLIs are installed: the `web` container. Users should be able to run T3 in multiple DDEV projects at once without manually coordinating ports.
...
Rationale: DDEV add-ons commonly use `.env.<addon>` files for user overrides, while JSON is a better fit for the nested T3 server settings currently generated by the devcontainer feature.
...
- Use a shared global npm prefix, matching the devcontainer feature pattern.
- Ensure the T3 binary is available in the `web` container `PATH`.
- Fail the web image build clearly if npm is unavailable.

Rationale: the existing devcontainer feature installs T3 globally via npm, so the DDEV add-on should preserve the same distribution path unless T3 upstream changes its recommended installation method.
